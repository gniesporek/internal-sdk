...
void RenderStackSystem::Rect::draw(Vector2D p, Vector2D s, Colors c) { RenderObject_t o(EDrawType::RECT); o.p1 = ToVec(p); o.p2 = ToVec(p.x + s.x, p.y + s.y); o.col1 = c; renderStackSystem.vecDrawData.push_back(o); }
void RenderStackSystem::Rect::draw(int x, int y, int w, int h, Colors c) { Rect::draw({ (float)x,(float)y }, { (float)w,(float)h }, c); }
void RenderStackSystem::Rect::outline(Vector2D p, Vector2D s, Colors c, Colors out) { RenderObject_t o(EDrawType::RECT); o.p1 = ToVec(p); o.p2 = ToVec(p.x + s.x, p.y + s.y); o.col1 = c; o.col2 = out; o.hasOutline = true; renderStackSystem.vecDrawData.push_back(o); }
...
void RenderStackSystem::Rect::fill(Vector2D p, Vector2D s, Colors c) { RenderObject_t o(EDrawType::FILLED_RECT); o.p1 = ToVec(p); o.p2 = ToVec(p.x + s.x, p.y + s.y); o.col1 = c; renderStackSystem.vecDrawData.push_back(o); }
void RenderStackSystem::Rect::fill(int x, int y, int w, int h, Colors c) { Rect::fill({ (float)x,(float)y }, { (float)w,(float)h }, c); }
void RenderStackSystem::Rect::gradient(int x, int y, int w, int h, Colors c1, Colors c2, bool v) { RenderObject_t o(EDrawType::GRADIENT_RECT); o.p1 = ToVec(x, y); o.p2 = ToVec(x + w, y + h); o.col1 = c1; o.col2 = c2; o.vertical = v; renderStackSystem.vecDrawData.push_back(o); }

void RenderStackSystem::Line::draw(Vector2D s, Vector2D e, Colors c, float t) { RenderObject_t o(EDrawType::LINE); o.p1 = ToVec(s); o.p2 = ToVec(e); o.col1 = c; o.thickness = t; renderStackSystem.vecDrawData.push_back(o); }
...
void RenderStackSystem::Line::gradient(Vector2D s, Vector2D e, Colors c1, Colors c2, float t) { RenderObject_t o(EDrawType::GRADIENT_LINE); o.p1 = ToVec(s); o.p2 = ToVec(e); o.col1 = c1; o.col2 = c2; o.thickness = t; renderStackSystem.vecDrawData.push_back(o); }
...
void RenderStackSystem::Text::draw(ImFont* f, Vector2D p, const char* t, Colors c, bool cent) { RenderObject_t o(EDrawType::TEXT); o.font = f; o.p1 = ToVec(p); o.text = t; o.col1 = c; o.centered = cent; renderStackSystem.vecDrawData.push_back(o); }
...
void RenderStackSystem::Text::outline(ImFont* f, Vector2D p, const char* t, Colors c, Colors out, bool cent) { RenderObject_t o(EDrawType::TEXT_OUTLINED); o.font = f; o.p1 = ToVec(p); o.text = t; o.col1 = c; o.col2 = out; o.centered = cent; renderStackSystem.vecDrawData.push_back(o); }
...
void RenderStackSystem::Text::shadow(ImFont* f, Vector2D p, const char* t, Colors c, Colors s, float off, bool cent) { RenderObject_t o(EDrawType::TEXT_SHADOW); o.font = f; o.p1 = ToVec(p); o.text = t; o.col1 = c; o.col2 = s; o.offset = off; o.centered = cent; renderStackSystem.vecDrawData.push_back(o); }
...
void RenderStackSystem::Triangle::draw(Vector2D a, Vector2D b, Vector2D c, Colors col) { RenderObject_t o(EDrawType::TRIANGLE); o.p1 = ToVec(a); o.p2 = ToVec(b); o.p3 = ToVec(c); o.col1 = col; renderStackSystem.vecDrawData.push_back(o); }
...
void RenderStackSystem::Triangle::fill(Vector2D a, Vector2D b, Vector2D c, Colors col) { RenderObject_t o(EDrawType::FILLED_TRIANGLE); o.p1 = ToVec(a); o.p2 = ToVec(b); o.p3 = ToVec(c); o.col1 = col; renderStackSystem.vecDrawData.push_back(o); }
...
void RenderStackSystem::Triangle::outline(Vector2D a, Vector2D b, Vector2D c, Colors col, Colors out) { RenderObject_t o(EDrawType::TRIANGLE); o.p1 = ToVec(a); o.p2 = ToVec(b); o.p3 = ToVec(c); o.col1 = col; o.col2 = out; o.hasOutline = true; renderStackSystem.vecDrawData.push_back(o); }

void RenderStackSystem::Circle::draw(Vector2D p, float r, Colors c) { RenderObject_t o(EDrawType::CIRCLE); o.p1 = ToVec(p); o.radius = r; o.col1 = c; renderStackSystem.vecDrawData.push_back(o); }
void RenderStackSystem::Circle::draw(int x, int y, float r, Colors c) { Circle::draw({ (float)x,(float)y }, r, c); }
void RenderStackSystem::Circle::fill(Vector2D p, float r, Colors c) { RenderObject_t o(EDrawType::FILLED_CIRCLE); o.p1 = ToVec(p); o.radius = r; o.col1 = c; renderStackSystem.vecDrawData.push_back(o); }
void RenderStackSystem::Circle::fill(int x, int y, float r, Colors c) { Circle::fill({ (float)x,(float)y }, r, c); }
void RenderStackSystem::Circle::outline(Vector2D p, float r, Colors c, Colors out) { RenderObject_t o(EDrawType::CIRCLE); o.p1 = ToVec(p); o.radius = r; o.col1 = c; o.col2 = out; o.hasOutline = true; renderStackSystem.vecDrawData.push_back(o); }

void RenderStackSystem::Image::draw(ImTextureID t, Vector2D p, Vector2D s, Colors tint) { RenderObject_t o(EDrawType::IMAGE); o.texture = t; o.p1 = ToVec(p); o.p2 = ToVec(p.x + s.x, p.y + s.y); o.col1 = tint; renderStackSystem.vecDrawData.push_back(o); }
...
void RenderStackSystem::Image::outline(ImTextureID t, Vector2D p, Vector2D s, Colors tint, Colors out) { RenderObject_t o(EDrawType::IMAGE); o.texture = t; o.p1 = ToVec(p); o.p2 = ToVec(p.x + s.x, p.y + s.y); o.col1 = tint; o.col2 = out; o.hasOutline = true; renderStackSystem.vecDrawData.push_back(o); }
...
void DrawingSystem::Rect::outline(int x, int y, int width, int height, Colors color, Colors outlineColor) {
...
void DrawingSystem::Rect::fill(int x, int y, int width, int height, Colors color) {
    ImGui::GetBackgroundDrawList()->AddRectFilled(ImVec2(x, y), ImVec2(x + width, y + height), color.ToImColor());
}

void DrawingSystem::Rect::draw(int x, int y, int width, int height, Colors color) {
    ImGui::GetBackgroundDrawList()->AddRect(ImVec2(x, y), ImVec2(x + width, y + height), color.ToImColor());
}

void DrawingSystem::Rect::outline(Vector2D position, Vector2D size, Colors color, Colors outlineColor) {
    outline((int)position.x, (int)position.y, (int)size.x, (int)size.y, color, outlineColor);
}

void DrawingSystem::Rect::fill(Vector2D position, Vector2D size, Colors color) {
    fill((int)position.x, (int)position.y, (int)size.x, (int)size.y, color);
}

void DrawingSystem::Rect::draw(Vector2D position, Vector2D size, Colors color) {
    draw((int)position.x, (int)position.y, (int)size.x, (int)size.y, color);
}

void DrawingSystem::Line::draw(Vector2D start, Vector2D end, Colors color, float thickness) {
    ImGui::GetBackgroundDrawList()->AddLine(ImVec2(start.x, start.y), ImVec2(end.x, end.y), color.ToImColor(), thickness);
}

void DrawingSystem::Line::draw(int x1, int y1, int x2, int y2, Colors color, float thickness) {
    ImGui::GetBackgroundDrawList()->AddLine(ImVec2((float)x1, (float)y1), ImVec2((float)x2, (float)y2), color.ToImColor(), thickness);
}

void DrawingSystem::Line::gradient(Vector2D start, Vector2D end, Colors startColor, Colors endColor, float thickness) {
...
void DrawingSystem::Line::gradient(int x1, int y1, int x2, int y2, Colors startColor, Colors endColor, float thickness) {
    gradient(Vector2D{ (float)x1, (float)y1 }, Vector2D{ (float)x2, (float)y2 }, startColor, endColor, thickness);
}

void DrawingSystem::Text::draw(ImFont* font, Vector2D position, const char* text, Colors color, bool centered) {
    if (!font || !text) return;
    ImGui::PushFont(font);
...
void DrawingSystem::Text::draw(ImFont* font, int x, int y, const char* text, Colors color, bool centered) {
    draw(font, Vector2D{ (float)x, (float)y }, text, color, centered);
}

void DrawingSystem::Text::outline(ImFont* font, Vector2D position, const char* text, Colors colText, Colors colOutline, bool centered) {
    if (!font || !text) return;
    ImGui::PushFont(font);
    ImVec2 textSize = ImGui::CalcTextSize(text);
    ImVec2 pos = centered ? ImVec2(position.x - textSize.x / 2.f, position.y - textSize.y / 2.f) : ImVec2(position.x, position.y);
...
void DrawingSystem::Text::outline(ImFont* font, int x, int y, const char* text, Colors colText, Colors colOutline, bool centered) {
    outline(font, Vector2D{ (float)x, (float)y }, text, colText, colOutline, centered);
}

void DrawingSystem::Text::shadow(ImFont* font, Vector2D position, const char* text, Colors textColor, Colors shadowColor, float offset, bool centered) {
    if (!font || !text) return;
    ImGui::PushFont(font);
    ImVec2 textSize = ImGui::CalcTextSize(text);
    ImVec2 pos = centered ? ImVec2(position.x - textSize.x / 2.f, position.y - textSize.y / 2.f) : ImVec2(position.x, position.y);
...
void DrawingSystem::Text::shadow(ImFont* font, int x, int y, const char* text, Colors textColor, Colors shadowColor, float offset, bool centered) {
    shadow(font, Vector2D{ (float)x, (float)y }, text, textColor, shadowColor, offset, centered);
}

int DrawingSystem::Text::getTextDimensions(ImFont* font, const char* text, bool width) {
...
int DrawingSystem::Text::getTextWidth(ImFont* font, const char* text) {
    return getTextDimensions(font, text, true);
}

int DrawingSystem::Text::getTextHeight(ImFont* font, const char* text) {
    return getTextDimensions(font, text, false);
}

void DrawingSystem::Image::draw(ID3D11ShaderResourceView* texture, Vector2D position, Vector2D size, Colors tint) {
    if (!texture) return;

    Vector2D tex_size =  drawingSystem.getTextureSize(texture);
...
void DrawingSystem::Image::draw(ID3D11ShaderResourceView* texture, int x, int y, int width, int height, Colors tint) {
    draw(texture, Vector2D{ (float)x, (float)y }, Vector2D{ (float)width, (float)height }, tint);
}

Vector2D DrawingSystem::getTextureSize(ID3D11ShaderResourceView* texture)
{
